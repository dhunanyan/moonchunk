lexer grammar MoonChunkLexer;

CHUNK       : 'chunk' ;
EXPORT      : 'export' ;
MOON        : 'moon' ;
INCLUDE     : '@include' ;
AS          : 'as' ;
FROM        : 'from' ;
IMPORT      : 'import' ;
OUTPUT      : 'output' ;
ENV         : 'env' ;
GLOBAL      : 'global' ;
PAGE        : 'page' ;
CONST       : 'const' ;
LET         : 'let' ;
FUNCTION    : 'function' ;
RETURN      : 'return' ;
FOR         : 'for' ;
WHILE       : 'while' ;
BREAK       : 'break' ;
CONTINUE    : 'continue' ;
IN          : 'in' ;
PARENT      : 'parent' ;
IF          : 'if' ;
ELSE        : 'else' ;
OR          : 'or' ;
AND         : 'and' ;
NOT         : 'not' ;
TRUE        : 'true' ;
FALSE       : 'false' ;
TYPE_INT    : 'int' ;
TYPE_FLOAT  : 'float' ;
TYPE_DOUBLE : 'double' ;
TYPE_BOOL   : 'bool' ;
TYPE_STRING : 'string' ;
TYPE_NUMBER : 'number' ;
TYPE_OBJECT : 'object' ;
TYPE_ARRAY  : 'array' ;
TYPE_NULL   : 'null' ;
TYPE_UNDEFINED: 'undefined' ;
TYPE_UNKNOWN: 'unknown' ;
TYPE_ANY    : 'any' ;
TYPE_VOID   : 'void' ;
META_LANG               : 'lang' ;
META_DIR                : 'dir' ;
META_HTML_CLASS         : 'htmlClass' ;
META_CHARSET            : 'charset' ;
META_VIEWPORT           : 'viewport' ;
META_TITLE              : 'title' ;
META_DESCRIPTION        : 'metaDescription' ;
META_KEYWORDS           : 'metaKeywords' ;
META_AUTHOR             : 'metaAuthor' ;
META_ROBOTS             : 'metaRobots' ;
META_THEME_COLOR        : 'themeColor' ;
META_CANONICAL_URL      : 'canonicalUrl' ;
META_FAVICON_HREF       : 'faviconHref' ;
META_APPLE_TOUCH_ICON   : 'appleTouchIconHref' ;
META_MANIFEST_HREF      : 'manifestHref' ;
META_OG_TYPE            : 'ogType' ;
META_OG_TITLE           : 'ogTitle' ;
META_OG_DESCRIPTION     : 'ogDescription' ;
META_OG_IMAGE           : 'ogImage' ;
META_OG_URL             : 'ogUrl' ;
META_OG_SITE_NAME       : 'ogSiteName' ;
META_OG_LOCALE          : 'ogLocale' ;
META_TWITTER_CARD       : 'twitterCard' ;
META_TWITTER_SITE       : 'twitterSite' ;
META_TWITTER_CREATOR    : 'twitterCreator' ;
META_TWITTER_TITLE      : 'twitterTitle' ;
META_TWITTER_DESCRIPTION: 'twitterDescription' ;
META_TWITTER_IMAGE      : 'twitterImage' ;
META_PRELOAD_LINKS      : 'preloadLinks' ;
META_PRECONNECT_LINKS   : 'preconnectLinks' ;
META_STYLES             : 'styles' ;
META_HEAD_SCRIPTS       : 'headScripts' ;
META_HEAD_EXTRA         : 'headExtra' ;
META_BODY_CLASS         : 'bodyClass' ;
META_PAGE_ID            : 'pageId' ;
META_TOP_BAR            : 'topBar' ;
META_HEADER             : 'header' ;
META_FOOTER             : 'footer' ;
META_MODALS             : 'modals' ;
META_SCRIPTS            : 'scripts' ;
META_BODY_END_EXTRA     : 'bodyEndExtra' ;

ARROW   : '=>' ;
EQ      : '==' ;
NEQ     : '!=' ;
ASSIGN  : '=' ;
PLUSPLUS: '++' ;
PLUS    : '+' ;
MINUS   : '-' ;
STAR    : '*' ;
SLASH   : '/' ;
PERCENT : '%' ;
LT      : '<' ;
GT      : '>' ;
LTE     : '<=' ;
GTE     : '>=' ;
DOT     : '.' ;
SCOPE   : '::' ;
COMMA   : ',' ;
COLON   : ':' ;
QUESTION: '?' ;
LPAREN  : '(' ;
RPAREN  : ')' ;
LBRACKET: '[' ;
RBRACKET: ']' ;
LBRACE  : '{' ;
RBRACE  : '}' ;
SEMI    : ';' ;

CONTENT_START
  : 'content' [ \t\r\n]* '{' -> pushMode(CONTENT_MODE)
  ;

STRING
  : '"' ( '\\' . | ~["\\\r\n] )* '"'
  ;

NUMBER
  : [0-9]+ ('.' [0-9]+)? [fFdD]?
  ;

IDENTIFIER
  : [A-Za-z_] [A-Za-z0-9_]*
  ;

LINE_COMMENT
  : '//' ~[\r\n]* -> skip
  ;

WS
  : [ \t\r\n\f]+ -> skip
  ;

mode CONTENT_MODE;

CONTENT_END
  : [ \t\r\n\f]* '}' [ \t\r\n\f]* ';' -> popMode
  ;

CONTENT_LT
  : '<' -> type(LT), pushMode(CONTENT_TAG_MODE)
  ;

CONTENT_LBRACE
  : '{' -> type(LBRACE), pushMode(CONTENT_EXPR_MODE)
  ;

CONTENT_TEXT
  : ~[<{}]+ 
  ;

mode CONTENT_TAG_MODE;

TAG_GT
  : '>' -> type(GT), popMode
  ;

TAG_SLASH
  : '/' -> type(SLASH)
  ;

TAG_ASSIGN
  : '=' -> type(ASSIGN)
  ;

TAG_IDENTIFIER
  : [A-Za-z_] [A-Za-z0-9_:-]* -> type(IDENTIFIER)
  ;

TAG_STRING
  : '"' ( '\\' . | ~["\\\r\n] )* '"' -> type(STRING)
  ;

TAG_LBRACE
  : '{' -> type(LBRACE), pushMode(CONTENT_EXPR_MODE)
  ;

TAG_WS
  : [ \t\r\n\f]+ -> skip
  ;

mode CONTENT_EXPR_MODE;

EXPR_RBRACE
  : '}' -> type(RBRACE), popMode
  ;

EXPR_LBRACE
  : '{' -> type(LBRACE), pushMode(CONTENT_EXPR_MODE)
  ;

EXPR_ARROW   : '=>' -> type(ARROW) ;
EXPR_EQ      : '==' -> type(EQ) ;
EXPR_NEQ     : '!=' -> type(NEQ) ;
EXPR_LTE     : '<=' -> type(LTE) ;
EXPR_GTE     : '>=' -> type(GTE) ;
EXPR_ASSIGN  : '=' -> type(ASSIGN) ;
EXPR_PLUSPLUS: '++' -> type(PLUSPLUS) ;
EXPR_PLUS    : '+' -> type(PLUS) ;
EXPR_MINUS   : '-' -> type(MINUS) ;
EXPR_STAR    : '*' -> type(STAR) ;
EXPR_SLASH   : '/' -> type(SLASH) ;
EXPR_PERCENT : '%' -> type(PERCENT) ;
EXPR_LT      : '<' -> type(LT) ;
EXPR_GT      : '>' -> type(GT) ;
EXPR_DOT     : '.' -> type(DOT) ;
EXPR_SCOPE   : '::' -> type(SCOPE) ;
EXPR_COMMA   : ',' -> type(COMMA) ;
EXPR_COLON   : ':' -> type(COLON) ;
EXPR_QUESTION: '?' -> type(QUESTION) ;
EXPR_LPAREN  : '(' -> type(LPAREN) ;
EXPR_RPAREN  : ')' -> type(RPAREN) ;
EXPR_LBRACKET: '[' -> type(LBRACKET) ;
EXPR_RBRACKET: ']' -> type(RBRACKET) ;
EXPR_SEMI    : ';' -> type(SEMI) ;

EXPR_CONST       : 'const' -> type(CONST) ;
EXPR_LET         : 'let' -> type(LET) ;
EXPR_FUNCTION    : 'function' -> type(FUNCTION) ;
EXPR_RETURN      : 'return' -> type(RETURN) ;
EXPR_WHILE       : 'while' -> type(WHILE) ;
EXPR_FOR         : 'for' -> type(FOR) ;
EXPR_BREAK       : 'break' -> type(BREAK) ;
EXPR_CONTINUE    : 'continue' -> type(CONTINUE) ;
EXPR_IN          : 'in' -> type(IN) ;
EXPR_PARENT      : 'parent' -> type(PARENT) ;
EXPR_AS          : 'as' -> type(AS) ;
EXPR_IF          : 'if' -> type(IF) ;
EXPR_ELSE        : 'else' -> type(ELSE) ;
EXPR_OR          : 'or' -> type(OR) ;
EXPR_AND         : 'and' -> type(AND) ;
EXPR_NOT         : 'not' -> type(NOT) ;
EXPR_TRUE        : 'true' -> type(TRUE) ;
EXPR_FALSE       : 'false' -> type(FALSE) ;
EXPR_TYPE_INT    : 'int' -> type(TYPE_INT) ;
EXPR_TYPE_FLOAT  : 'float' -> type(TYPE_FLOAT) ;
EXPR_TYPE_DOUBLE : 'double' -> type(TYPE_DOUBLE) ;
EXPR_TYPE_BOOL   : 'bool' -> type(TYPE_BOOL) ;
EXPR_TYPE_STRING : 'string' -> type(TYPE_STRING) ;
EXPR_TYPE_NUMBER : 'number' -> type(TYPE_NUMBER) ;
EXPR_TYPE_OBJECT : 'object' -> type(TYPE_OBJECT) ;
EXPR_TYPE_ARRAY  : 'array' -> type(TYPE_ARRAY) ;
EXPR_TYPE_NULL   : 'null' -> type(TYPE_NULL) ;
EXPR_TYPE_UNDEFINED: 'undefined' -> type(TYPE_UNDEFINED) ;
EXPR_TYPE_UNKNOWN: 'unknown' -> type(TYPE_UNKNOWN) ;
EXPR_TYPE_ANY    : 'any' -> type(TYPE_ANY) ;
EXPR_TYPE_VOID   : 'void' -> type(TYPE_VOID) ;

EXPR_STRING
  : '"' ( '\\' . | ~["\\\r\n] )* '"' -> type(STRING)
  ;

EXPR_NUMBER
  : [0-9]+ ('.' [0-9]+)? [fFdD]? -> type(NUMBER)
  ;

EXPR_IDENTIFIER
  : [A-Za-z_] [A-Za-z0-9_]* -> type(IDENTIFIER)
  ;

EXPR_LINE_COMMENT
  : '//' ~[\r\n]* -> skip
  ;

EXPR_WS
  : [ \t\r\n\f]+ -> skip
  ;
